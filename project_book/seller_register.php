...
if(isset($_POST['submit'])){
   ...
   if(!preg_match('/@ugrad\.iiuc\.ac\.bd$/', $email)){
      ...
      $check = mysqli_query($conn, "SELECT * FROM `seller` WHERE email = '$email' OR seller_id = '$seller_id'") or die('query failed');
      ...
      if(mysqli_num_rows($check) > 0){
         ...
         mysqli_query($conn, "INSERT INTO `seller` (seller_id, name, email, password) VALUES('$seller_id', '$name', '$email', '$pass')") or die('query failed');
...
<head>
   ...
   <title>Seller Registration - IIUC Portal</title>
   <style>
...
<body>
   ...
   <div class="login container grid">
      <div class="login__access">
         <h1 class="login__title">Register as Seller</h1>

         <?php if(isset($message)): ?>
            <div class="message <?php echo (end($message) === 'Seller registered successfully!') ? 'success' : ''; ?>">
               <span><?php echo end($message); ?></span>
            ...
            <form action="" method="post" class="login__form">
               <div class="login__content grid">
                  <div class="login__box">
                     ...
                     <label for="seller_id" class="login__label">Seller ID (Student ID)</label>
               ...
               <button type="submit" name="submit" class="login__button">Register Now</button>
            </form>

            <p class="login__switch">
               Already have an account?
               <a href="seller_login.php" id="loginButtonRegister">Login</a>
            </p>
         </div>
      </div>
   </div>
